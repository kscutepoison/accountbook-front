<template>
      ...
      <v-list-group v-for="(accounts, type) in accountsByType" :value="type">
        <template v-slot:activator="{ props }">
          <v-list-item v-bind="props" :title="type">
            ...
            <template v-slot:append>
              <v-chip
                :color="getBalancesByTypeSum(type) >= 0 ? 'green' : 'red'"
              >
  ...
  <v-dialog width="auto" scrollable v-model="isShowAccountInput" persistent>
    ...
    <v-btn @click="isShowAccountInput = false"> 閉じる </v-btn>
  </v-dialog>
</template>
...
const accountsByType = computed(() => {
  const accounts = accountsWithBalanceSum.value.filter((account) => {
    ...
    if (searchItems.value.account_name) {
      res.push(
        account.account_name
          .toLowerCase()
          .indexOf(searchItems.value.account_name.toLowerCase()) >= 0
      );
    }
    ...
    if (searchItems.value.owner && searchItems.value.owner.length > 0) {
      res.push(
        searchItems.value.owner.some((owner) => {
    ...
    if (
      ...
      res.push(
        searchItems.value.is_archived.some((is_archived) => {
    ...
    if (
      ...
      res.push(
        searchItems.value.is_primary.some((is_primary) => {
...
const accountTypes = computed(() => {
  return Array.from(
    new Set(accountsWithBalanceSum.value.map((account) => account.type))
  );
});

const owners = computed(() => {
  return Array.from(
    new Set(accountsWithBalanceSum.value.map((account) => account.owner))
  );
});
